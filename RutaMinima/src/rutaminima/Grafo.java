...
public class Grafo {
    ...
    public void mostrarAristas(JTable tbl) {
     ...
     // Método para calcular el grado de todos los nodos en el grafo y mostrarlo en la tabla
    public DefaultTableModel calcularGradoDeNodos() {
        DefaultTableModel modelo = new DefaultTableModel(new Object[]{"Vértice", "Grado"}, 0);

        for (Nodo nodo : nodos) {
            int grado = nodo.getVecinos().size();
            modelo.addRow(new Object[]{nodo.getNombre(), grado});
        }
    ...
    private void dfs(Nodo inicio, Set<Nodo> visitados) {
    ...
    public boolean esConexo() {
        ...
        Nodo nodoInicio = nodos.get(0); // Tomar el primer nodo como inicio
